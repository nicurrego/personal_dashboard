...
function KPICard({ label, value, subtext, trend, trendValue, inverseTrend = false, accent = 'cobalt' }: { 
  ...
  let trendColor = 'text-secondary-text';
  if (trend === 'up') trendColor = inverseTrend ? 'text-electric-orange' : 'text-acid-green';
  if (trend === 'down') trendColor = inverseTrend ? 'text-acid-green' : 'text-electric-orange';
  if (trend === 'neutral') trendColor = 'text-cobalt-blue';

  const accentClass = accent === 'green' ? 'border-l-acid-green' : accent === 'orange' ? 'border-l-electric-orange' : 'border-l-cobalt-blue';
